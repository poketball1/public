...
> **2026-07-13 역사 자료.** 아래 모델 배치·은퇴 전제·내부 경로는 당시 기록이며
> 현재 설치 지침이 아니다. 현재 공개판은 [Boss Mode 스킬](skills/boss-mode/SKILL.md)과
> [설치·운영 매뉴얼](README.md)을 사용한다. 특히 Fable 은퇴를 현행 전제로 삼지 않는다.
...
아래 자료와 접힌 전문은 2026-07-13 운용 기록이다. 요약 카드만 보고 원문을 숨기지 않도록 문서
상단에 전문을 함께 실었다. 별도 파일도 같은 공개 저장소 안에 있으므로 그대로
복사하거나 diff로 비교할 수 있다.

- [현재 공개판 Boss Mode skill](skills/boss-mode/SKILL.md) — 아래 접힌 역사 원문과 별도
- [Fable main 전담 지침 원문 파일](source-material/fable-main-instructions-2026-07-13.md)

당시 외부 환경에 안내했던 설치 순서는 다음과 같다. 신규 설치에는 위의 현재 매뉴얼을 적용한다.
...
이하 해설도 당시 운영을 설명하는 역사 자료다. 현재 호출·설치 지침은
[공개 스킬과 설치 매뉴얼](README.md)을 따른다.
